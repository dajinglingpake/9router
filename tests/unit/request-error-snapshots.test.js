...
import { getRequestLogContext } from "../../src/lib/requestCaller.js";
...
it("captures elapsed time and the active request fields without copying secrets or bodies", () => {
  const snapshot = getRequestLogContext({
    ...
    requestBytes: 2048, startedAt: 1000, deadline: 5000, state: "running", waitMs: 0,
  }, 2500);
...
it.each(["timeout", "cancel"])("captures the visible queue position before %s removes the request", async (action) => {
  ...
  const metadata = { providerScope: "codex", routingModel: "sol", requestedModel: "client-alias", upstreamModel: "sol", thinkingLevel: "high", requestBytes: 2048, stream: true, apiKeyName: "客户端甲", clientIp: "192.0.2.10", endpoint: "/v1/responses" };
