...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("keeps Claude system in user content without sending an unsupported top-level field", () => {
    const out = C2K({
  ...
  it("replays the system instruction inside the frozen first user turn", () => {
    const credentials = { rawHeaders: { "x-session-id": "stable-system-replay" }, connectionId: "kiro-system-account" };
    const first = C2K({
