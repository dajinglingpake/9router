...
export function supportsConnectionUsage(connection) {
  return USAGE_SUPPORTED_PROVIDERS.includes(connection.provider) && (connection.authType === "oauth" ||
    (["apikey", "api_key"].includes(connection.authType) && USAGE_APIKEY_PROVIDERS.includes(connection.provider)));
}

export async function loadConnectionUsage(connection, { force = false, signal } = {}) {
  if (!supportsConnectionUsage(connection)) return { message: "Usage not available for this connection" };
  const proxyConfig = await resolveConnectionProxyConfig(connection.providerSpecificData);
  const proxyOptions = { ...proxyConfig, strictProxy: proxyConfig.connectionProxyEnabled === true, signal };
  if (connection.authType === "oauth") {
    try {
      const result = await refreshAndUpdateCredentials(connection, false, proxyOptions);
      connection = result.connection;
    } catch (error) {
      throw Object.assign(new Error(`Credential refresh failed: ${error.message}`), { status: 401 });
    }
  }
  let usage = await getUsageForProvider(connection, proxyOptions, { force });
  if (connection.authType === "oauth" && isAuthExpiredMessage(usage) && connection.refreshToken) {
    try {
      const result = await refreshAndUpdateCredentials(connection, true, proxyOptions);
      usage = await getUsageForProvider(result.connection, proxyOptions, { force });
    } catch {
      console.warn(`[Usage] ${connection.provider}: force refresh failed`);
    }
  }
  return usage;
}
