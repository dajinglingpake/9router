 ...
 */
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, apiKey = null, seenModels = new Set(), excludedAccounts = new Set(), deadline = Date.now() + queueTimeoutMs(), retryCount = 0, comboNames = []) {
  ...
  if (comboModels?.length) {
    return handleSingleModelChat(body, comboModels[0], clientRawRequest, request, apiKey, seenModels, excludedAccounts, deadline, retryCount, [...comboNames, modelStr]);
  }
  ...
  let { model } = modelInfo;
  const modelByConnection = await getComboAccountModels(comboNames, provider);
  ...
  try {
    ...
    if (!liveConnection || !liveConnection.isActive || isModelLockActive(liveConnection, model)) {
      if (!sessionKey) {
        ...
        return handleSingleModelChat(body, modelStr, clientRawRequest, request, apiKey, new Set(), excludedAccounts, deadline, retryCount, comboNames);
      }
