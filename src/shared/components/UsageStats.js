...
function sortData(dataMap, pendingMap = {}, sortBy, sortOrder, viewMode = "costs") {
  return Object.entries(dataMap || {})
    .map(([key, data]) => {
      ...
      // Cost split is a token-share allocation of the server total. Cached
      // tokens are a subset of prompt tokens, so show them separately.
      ...
      return { ...data, key, totalTokens, totalCost, inputCost, cachedCost, outputCost, usageShare, pending: pendingMap[key] || 0 };
    })
    .sort((a, b) => {
      let valA = a[sortBy];
      let valB = b[sortBy];
      ...
      case "clientIp": {
        return {
          ...
          renderSummaryCells: (group) => {
            ...
            return (
              <>
                <td className="px-6 py-3">{item?.keyName || <span className="text-text-muted">—</span>}</td>
                <td className="px-6 py-3">{item?.rawModel || <span className="text-text-muted">—</span>}</td>
                <td className="px-6 py-3">
                  {item ? <Badge variant="neutral" size="sm">{item.provider || "—"}</Badge> : <span className="text-text-muted">—</span>}
                </td>
