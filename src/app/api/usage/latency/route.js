...
export async function GET(request) {
  try {
    ...
    const result = await getRequestDetails({ page: 1, pageSize: 1000, startDate: getStartDate(period) });
    ...
    const data = Array.from({ length: buckets.count }, (_, index) => {
      ...
      for (const model of models) {
        ...
        point[model] = item ? Math.round(item.totalLatency / item.requests) : null;
      }
      return point;
    });
