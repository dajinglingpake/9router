  ...
  return (
    <div data-i18n-skip className={`rounded-md border p-3 ${waiting ? "border-amber-200 bg-amber-50/60" : "border-border-subtle bg-surface"}`}>
      ...
      <dl className="grid gap-x-4 gap-y-1 sm:grid-cols-2">
        ...
        <div title="fast / priority 表示请求开启快速模式；发送参数不代表上游保证采用。"><dt className="inline">Fast 模式：</dt><dd className="inline text-text-main">{["fast", "priority"].includes(item.requestedServiceTier) ? "开启" : item.requestedServiceTier === "unspecified" ? "未指定" : item.requestedServiceTier || "未记录"} · 客户端 {item.requestedServiceTier === "unspecified" ? "未指定" : item.requestedServiceTier || "未记录"} → 上游 {item.upstreamServiceTier === "unspecified" ? "未指定" : item.upstreamServiceTier || (failed ? "未记录" : "待发送")}</dd></div>
